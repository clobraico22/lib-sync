...
def get_spotify_matches(rekordbox_collection: RekordboxCollection) -> dict[str, str]:
    ...
    for rb_track in rekordbox_collection:
        ...
        best_match_uri = find_best_track_match_uri(rb_track,spotify_search_track_results['items'])
        while best_match_uri=='next' and spotify_search_track_results['next']:
            spotify_search_track_results = spotify.next(spotify_search_track_results)['tracks']
            best_match = find_best_track_match_uri(rb_track,spotify_search_track_results["items"])

        if best_match_uri is None:
            logging.warning(
                f"could not find a good match for the track {rb_track}"
            )
...
def export_failed_matches_to_file(failed_matches: list[RekordboxTrack]):
    ...
    with open(f'failed_matches_{datetime.now()}.txt','w') as fp:
        fp.write("The below files were not found on Spotify. Consider updating the metadata before re-running lib-sync.\n")
...
# NOTE: WIP!!
def find_best_track_match_uri(rekordbox_track: RekordboxTrack,list_of_spotify_tracks: list):
    ...
    if not list_of_spotify_tracks:
        logging.warning(
            f"list_of_spotify_tracks is empty. Returning None...\n"
        )
        return None
