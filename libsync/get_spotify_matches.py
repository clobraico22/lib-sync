...
def get_spotify_matches(rekordbox_collection: RekordboxCollection) -> dict[str, str]:
    ...
    for rb_track in rekordbox_collection:
        ...
        list_of_spotify_tracks = spotify_search_track_results[
            "items"
        ]  # this will give us the first page of results
        best_match = find_best_track_match(rb_track, list_of_spotify_tracks)
        match_map[rb_track] = best_match[
            "uri"
        ]  # NOTE: consider returning the entire dictionary rather than just uri
...
def search_spotify_for_rekordbox_track(
    ...
    if all(value is None for value in filter_map.values()):
        raise ValueError(
            "The given rekordbox track is empty."
            + "At least one of the following fields must not be None: 'artist', 'track', 'album'"
        )
    filter_list = [f"{key}:{value}" for key, value in filter_map.items()]
